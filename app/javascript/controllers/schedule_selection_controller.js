...
export default class extends Controller {
  ...
  static values = { src: String, frameId: { type: String, default: "modal_frame" } }
  ...
  onSubmitSuccess() {
    if (this.hasFrameTarget) {
      ...
      fetch(`/availability_schedules.json?availability_id=${this.frameTarget.dataset.availabilityId}`)
        .then(response => response.json())
        .then(data => {
          data.forEach(schedule => { // Check for schedules that are not listed in the form and add them
            if (this.scheduleIdInputTargets.find(input => parseInt(input.dataset.savedScheduleId) === schedule.id)) return;
            let event = new Event("", undefined);
            Object.defineProperty(event, 'target', {value: {dataset: {scheduleId: schedule.id, scheduleName: schedule.name}}});
  ...
  addSchedule(event, existing = false) {
    ...
    let newScheduleInput = this.scheduleIdInputTargets.find(
      scheduleIdInput => !this.isMarkedForRemoval(scheduleIdInput) && scheduleIdInput.dataset.savedScheduleId === "0"
    );
    ...
    if (existing) {
      newScheduleInput.closest(".nested-form-wrapper").dataset.newRecord = "false";
    }
  ...
  removeSchedule(event) {
    ...
    const existingInput = this.scheduleIdInputTargets.find(
      scheduleIdInput => !this.isMarkedForRemoval(scheduleIdInput) && scheduleIdInput.dataset.savedScheduleId === targetScheduleId
    );
    ...
    const checkbox = this.scheduleToggleTargets.find(toggle => toggle.dataset.scheduleId === event.originalTarget.dataset.removeId);
  ...
  isMarkedForRemoval(scheduleIdInput) {
    return scheduleIdInput.closest(".nested-form-wrapper").style.display === "none";
  }
  ...
  updateCalendar() {
    ...
    this.calendarOutlet.replaceEventSource(id, {
      url: "/calendars",
      method: 'GET',
      extraParams: { schedule_ids: this.scheduleIdInputTargets.filter(input => !this.isMarkedForRemoval(input))
  ...
  scheduleToggleTargetConnected(element) {
    ...
    const existingInput = this.scheduleIdInputTargets.find(
      scheduleIdInput => !this.isMarkedForRemoval(scheduleIdInput) && scheduleIdInput.dataset.savedScheduleId === scheduleId
    );
